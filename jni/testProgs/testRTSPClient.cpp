      ...
      else
      {
          ...
          if ((nal_unit == 1 || nal_unit == 5) && fReceivedIDR >=  2)
          {
              ...
              if (idx < INBUF_CNT && idx >= 0)
              {
                  ...
                  jenv->CallVoidMethod(jobject_rtspcodec, methodID_vdecode, frameSize+4,nal_unit, i_pts);
...
void DummySink::initJni(JNIEnv* env, jobject obj) {
	if (!bJniMethodInited) {
		...
		methodID_vdecode = jenv->GetMethodID(jclass_rtspcodec, "VideoDecodeCallback", "(IIJ)V");
